...
def substitute_expr(expr, subs_dict):
    """Substitute the expression with the dictionary of substitutions
    inputs: expr is the expression
            subs_dict is the dictionary of substitutions
    output: expr_subs is the expression after substitution"""
...
def gen_HM(k, S, params):
    ...
    XdX = [sp.Symbol('XdX%d' % (i * 2 * nspins + j), commutative=True) 

...
def atom_pos():
    ...
    y = 0.3646
    z = 0.7545
    atom1 = np.mat([x*la, y*lb, z*lc])
    atom2 = np.mat([(1 - x)*la, (1 - y)*lb, z*lc])
    atom3 = np.mat([x*la, (y + 1/2)*lb, (z - 1/2)*lc])
    atom4 = np.mat([(1 - x)*la, (-y + 1/2)*lb, (z - 1/2)*lc])
    atom5 = np.mat([(x + 1/2)*la, y*lb, (z - 1/2)*lc])
    atom6 = np.mat([(-x + 1/2)*la, (1 - y)*lb, (z - 1/2)*lc])
    atom7 = np.mat([(x + 1/2)*la, (y + 1/2)*lb, z*lc])
    atom8 = np.mat([(-x + 1/2)*la, (-y + 1/2)*lb, z*lc])
    atom9 = np.mat([(x + 1/4)*la, (-y + 1/4 + 1)*lb, (z + 1/4)*lc])
    atom10 = np.mat([(-x + 1/4)*la, (y + 1/4)*lb, (z + 1/4)*lc])
    atom11 = np.mat([(x + 1/4)*la, (-y + 3/4)*lb, (z + 3/4 - 1)*lc])
    atom12 = np.mat([(-x + 1/4)*la, (y + 3/4)*lb, (z + 3/4 - 1)*lc])
    atom13 = np.mat([(x + 3/4)*la, (-y + 1/4 + 1)*lb, (z + 3/4 - 1)*lc])
    atom14 = np.mat([(-x + 3/4)*la, (y + 1/4)*lb, (z + 3/4 - 1)*lc])
    atom15 = np.mat([(x + 3/4)*la, (-y + 3/4)*lb, (z + 1/4)*lc])
    atom16 = np.mat([(-x + 3/4)*la, (y + 3/4 - 1)*lb, (z + 1/4)*lc])
...
def atom_pos_ouc():
    ...
    r_pos_ouc.append(apos[8])
    r_pos_ouc.append(apos[9])
    r_pos_ouc.append(apos[10])
    r_pos_ouc.append(apos[11])
    r_pos_ouc.append(apos[12])
    r_pos_ouc.append(apos[13])
    r_pos_ouc.append(apos[14])
    r_pos_ouc.append(apos[15])
    for i in range(-1, 2):
        for j in range(-1, 2):
            for k in range(-1, 2):
                if i == 0 and j == 0 and k == 0:  # first set of spins are in the unit cell
                    ...
                    for at in range(len(apos)):
                        r_pos_ij = apos[at]+i*uc[0]+j*uc[1]+k*uc[2]
...
def spin_interactions(p):
    ...
    for i in range(Nspin):
        for j in range(Nspin_ouc):
            if np.round_(la.norm(apos[i]-apos_ouc[j]), 2) == np.round_(neighbor_dist_list[0], 2):
                Jex[i, j] = J1
                Gex[i, j] = G1
            elif np.round_(la.norm(apos[i]-apos_ouc[j]), 2) == np.round_(neighbor_dist_list[1], 2):
                Jex[i, j] = J2
                Gex[i, j] = G2
            elif np.round_(la.norm(apos[i]-apos_ouc[j]), 2) == np.round_(neighbor_dist_list[2], 2):
                Jex[i, j] = J3
                Gex[i, j] = G3
            else:
                Jex[i, j] = 0.0
